...
let hierarchical ~build_mode:_ _scope (i : Signal.t I.t) =
  ...
  <-- reg
        reg_spec
        ~enable:i.update_en
        ~initialize_to:Lru.initial_state.grid
        ~clear_to:(Signal.of_bits Lru.initial_state.grid)
        next_state;
  { O.grid = state_signal; least_row = Lru.least_row { Lru.State.grid = state_signal } }
;;
...
let%expect_test "lru matrix update manual" =
  create_sim (fun ~inputs ~outputs sim ->
    let (inputs : Bits.t ref I.t) = inputs in
    let (outputs : Bits.t ref O.t) = outputs in
    inputs.clock.clear := vdd;
    ...
    let update ~way =
      ...
      lr
    in
    ignore (update ~way:3 : int);
    ignore (update ~way:2 : int);
    ignore (update ~way:1 : int);
    ignore (update ~way:3 : int);
    ignore (update ~way:2 : int));
  [%expect
    {|
